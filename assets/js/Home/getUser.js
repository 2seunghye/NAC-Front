...
function printUser() {
  const userTableColumn = ["Network Name", "Object Name", "Object MacAddress", "Resource",
    "Action", "Permission", "Last Request Time", "Time of Unblock", "Min Interval Time (m)", "Number of Requests during Min Interval Time", "Threshold"];

  const keys = ["methodName", "object", "resource", "action", "permission", "toLR",
    "timeOfUnblock", "minInterval", "noFR", "threshold"];
  ...
  for (let i = 0, columnIndex = 0; i < keys.length; i++) {
    ...
    if (key == 'object') {
      for (let subKey of objKeys) {
        createDataList(userTableColumn[columnIndex++], policyTable[key][subKey]);
      }
    } else {
